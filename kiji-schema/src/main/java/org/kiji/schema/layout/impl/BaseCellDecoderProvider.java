 ...
 *   {@link #BaseCellDecoderProvider(org.kiji.schema.layout.KijiTableLayout, java.util.Map,
 ...
 */
@ApiAudience.Private
public final class BaseCellDecoderProvider implements CellDecoderProvider {

  private static final Logger LOG = LoggerFactory.getLogger(BaseCellDecoderProvider.class);
   ...
   */
  public BaseCellDecoderProvider(
  ...
  public <T> KijiCellDecoder<T> getDecoder(KijiColumnName column) {
    ...
    if (null != decoder) {
      ...
      return (KijiCellDecoder<T>) mColumnDecoderMap.get(
          new KijiColumnName(column.getFamily(), null));
    }
  }
