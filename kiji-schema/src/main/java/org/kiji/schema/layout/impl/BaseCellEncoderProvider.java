 ...
 */
@ApiAudience.Private
public final class BaseCellEncoderProvider implements CellEncoderProvider {

  /** Maps column names to encoders. */
  private final ImmutableMap<KijiColumnName, KijiCellEncoder> mEncoderMap;
   ...
   */
  public BaseCellEncoderProvider(
    ...
    final Map<KijiColumnName, KijiCellEncoder> encoderMap = Maps.newHashMap();
  ...
  /** {@inheritDoc} */
  @Override
  public KijiCellEncoder getEncoder(KijiColumnName column) {
    final KijiCellEncoder encoder = mEncoderMap.get(column);
    if (null != encoder) {
      return encoder;
    } else {
      return mEncoderMap.get(new KijiColumnName(column.getFamily(), null));
    }
  }
}
